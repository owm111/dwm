...
static void gridmode(Monitor *m);
...
winpid(Window w)
{
	...
	if (!r)
		return (pid_t)0;
	for (i = xcb_res_query_client_ids_ids_iterator(r); i.rem;
			xcb_res_client_id_value_next(&i)) {
		spec = i.data->spec;
		if (spec.mask & XCB_RES_CLIENT_ID_MASK_LOCAL_CLIENT_PID) {
			t = xcb_res_client_id_value_value(i.data);
			result = *t;
			break;
		}
	}
	...
	rc = XGetWindowProperty(dpy, w, XInternAtom(dpy, "_NET_WM_PID", 0),
			0, 1, False, AnyPropertyType,
			&type, &format, &len, &bytes, &prop);
        if (rc != Success || !prop)
               return 0;
        ret = *(pid_t*)prop;
        XFree(prop);
